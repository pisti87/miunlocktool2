...
class XiaomiUnlockTool:
    ...
    def create_material_ui(self):
        ...
        self.subtitle_label = MD3Label(
            title_frame,
            text="https://t.me/miunlocktoolnew",
    ...
    def update_ui_text(self):
        ...
        is_manual_before_translate = "ручной" in current_mode.lower() or "manual" in current_mode.lower()
        ...
        try:
            if self.settings.get('log_to_txt', False):
                if not hasattr(self, 'current_log_file') or self.current_log_file is None:
                    ...
                    self.current_log_file = os.path.join(log_dir, f"unlock_log_{date_str}.txt")
    ...
    def wait_for_presync_time(self):
        ...
        if current_hour < 23 or (current_hour == 23 and current_minute < 58):
            ...
            self.log_message("Время > 23:59. Выполняем синхронизацию времени перед стартом...", color="blue")
            new_time = self.request_handler.get_initial_beijing_time()
            if new_time:
                self.start_beijing_time = new_time
                self.start_timestamp = time.time()
                self.log_message("Время успешно синхронизировано!", color="green")
        ...
        self.log_message(f"Ожидание синхронизации в {target_sync_time.strftime('%H:%M:%S')} (Пекин)", color="blue")
        self.status_var.set(f"Ожидание {target_sync_time.strftime('%H:%M')} для синхронизации...")
        ...
        def check_presync():
            ...
            if current_time >= target_sync_time:
                self.log_message("Наступило время синхронизации! Выполняем точную синхронизацию времени...",
                                 color="blue")
                ...
                if new_time:
                    self.start_beijing_time = new_time
                    self.start_timestamp = time.time()
                    self.log_message(f"Время обновлено: {new_time.strftime('%H:%M:%S.%f')}", color="green")
                else:
                    self.log_message("Ошибка повторной синхронизации, продолжаем с текущим таймером.", color="red")
    ...
    def start_spam_attack(self):
        ...
        self.log_message(f"СПАМ РЕЖИМ: Целевые времена: {spam_times}", color="blue")
        self.status_var.set("Ожидание спам-атаки...")
    ...
    def schedule_spam_request(self, seq_num, offset_val, target_time, device_id):
        ...
        while True:
            ...
            if time_diff <= 0:
                self.log_message(f"[Спам #{seq_num}] Отправка запроса в 23:59:{offset_val:.1f}s", color="orange")
    ...
    def start_manual_mode(self):
        try:
            ...
            if script_time < 0 or script_time > 60:
                self.log_message("Некорректное время. Используйте значение от 0 до 60")
                messagebox.showerror("Ошибка", "Некорректное время. Используйте значение от 0 до 60")
                return
            self.log_message(self.translation.tr('manual_mode_start').format(script_time))
        except ValueError as e:
            self.log_message(f"Ошибка времени: {e}")
            messagebox.showerror("Ошибка", self.translation.tr('manual_time_error'))
            return
