...
class RequestHandler:
            ...
            try:
                ...
                if code == 0:
                    ...
                    if apply_result == 1:
                        ...
                        # Показываем только одно окно успеха
    ...
    def show_success_dialog(self):
        """Показывает только одно окно успеха"""
    ...
    def make_single_request_with_new_device_id(self, seq_num, offset_val, device_id):
        """Отправляет запрос с уникальным deviceId для каждого запроса"""
        try:
            cookie = self.app.cookie_value.get().strip()
            ...
            if response:
                try:
                    ...
                    prefix = self.app.translation.tr('log_spam_response_prefix').format(seq_num, f"{offset_val:.1f}")
                    ...
                    if code == 0 or code == 100003:
                        self.app.log_message(f"{self.app.translation.tr('log_spam_success_prefix')} {log_str}",
                                             color="green")
                        # Показываем окно успеха только для первого успешного запроса
                        if code == 0 or code == 100003:
                            self.show_success_dialog()
                    elif code == 100004 or code == 10004:
                        # Ошибка авторизации
