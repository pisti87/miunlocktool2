...
def get_or_create_device_id():
    ...
    try:
        ...
        # Генерируем новый ID из 12 случайных символов (буквы и цифры)
...
def get_system_info():
    ...
    # Получаем информацию о GPU (Windows)
    if os.name == 'nt':
        try:
            import wmi
            w = wmi.WMI()
...
class XiaomiUnlockTool:
    def __init__(self, root):
        ...
        # Сохраняем device_id в конфиг для отображения в настройках
        ...
        # Хранилище для предварительно сгенерированных device_id для спам-запросов
        self.spam_device_ids = []

        self.show_welcome_if_needed()

        # Отправляем информацию о запуске на сервер
        self.send_startup_info()

    def send_startup_info(self):
        """Отправляет информацию о запуске программы на сервер"""
    ...
    def open_login_dialog(self):
        """Открывает диалог входа в Xiaomi аккаунт"""
        try:
    ...
    def collect_and_send_logs(self):
        """Собирает логи из текстового поля и отправляет на сервер"""
        try:
            self.log_text.configure(state="normal")
    ...
    def start_process(self):
        ...
        # Генерируем deviceID для всех запросов ЗАРАНЕЕ
    ...
    def wait_for_presync_time(self):
        """Ожидание времени для измерения пинга в 23:59:50"""
    ...
    def measure_ping_and_proceed(self):
        """Измеряет пинг и запускает режим отправки"""
    ...
    def start_spam_attack(self, ping_ms):
        """Запуск спам-атаки с 2 запросами, используя предварительно сгенерированные device_id"""
    ...
    def schedule_spam_request(self, seq_num, send_seconds, target_time, device_id):
        """Планирует отправку спам-запроса на конкретное время"""
        while True:
            current_time = self.request_handler.get_synchronized_beijing_time(self.start_beijing_time,
                                                                              self.start_timestamp)
            time_diff = (target_time - current_time).total_seconds()
