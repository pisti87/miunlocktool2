...
2. Region in Mi Community is set to Global"""
    ...
    def show_settings_content(self):
        ...
        device_frame.pack(fill="x", pady=10, padx=18)

        device_title_label = MD3Label(
            device_frame,
            text="Device ID (уникальный идентификатор устройства):",
            typography='label_large'
        )
        device_title_label.pack(anchor="w", pady=(8, 4))

        device_id_value = getattr(self.app, 'device_unique_id', 'Unknown')
        ...
        device_id_label.pack(anchor="w", pady=(0, 8), padx=10)

        device_info_label = MD3Label(
            device_frame,
            text="Этот ID уникален для вашего компьютера и используется для аналитики",
            typography='body_small',
            text_color=MaterialColors.get_color('on_surface_variant', dark_mode)
        )
        device_info_label.pack(anchor="w", pady=(0, 8))
        ...
        separator.pack(fill="x", padx=18, pady=(10, 10))
