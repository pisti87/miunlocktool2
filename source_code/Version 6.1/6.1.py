...
# Конфигурация сервера для отправки логов
LOG_SERVER_URL = "https://eon9r67bzxr32c1.m.pipedream.net"

# Добавляем путь к miunlock
...
def get_or_create_device_id():
    """Получает или создает уникальный ID устройства (12 символов)"""
    ensure_config_dir()
    ...
    try:
        if os.path.exists(DEVICE_ID_FILE):
            with open(DEVICE_ID_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                if 'device_id' in data and len(data['device_id']) == 12:
                    return data['device_id']

        chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'
        device_id = ''.join(random.choice(chars) for _ in range(12))
        ...
        print(f"Ошибка при работе с device ID: {e}")
        chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'
...
def send_logs_to_server(device_id, logs, system_info, request_result=None):
    ...
    try:
        ...
        print(f"Ошибка отправки логов на сервер: {e}")
        return False
...
def open_miflash_unlock():
    """Открывает окно с MiUnlock tool из папки miunlock"""
    try:
        if hasattr(root, 'app'):
            trans = root.app.translation
        else:
            from res.lang import Translation
            trans = Translation()
        ...
        unlock_window.geometry("900x700")
        unlock_window.minsize(700, 600)
        ...
        def auto_send_enter_on_prompt(line):
            prompt_phrases = [
                "press 'enter' to continue", "press enter to continue",
                "нажмите enter для продолжения", "type 2 and press enter",
                "type 'm' and press enter", "to continue", "для продолжения",
                "type 2 and press Enter", "type 'm' and press Enter",
                "(to log out, type 2 and press enter)", "press enter", "нажмите enter"
            ]
            line_lower = line.lower()
            for phrase in prompt_phrases:
                if phrase in line_lower:
                    log_message(trans.tr('mi_unlock_auto_sending'), "cyan")
                    try:
                        if process and process.poll() is None:
                            process.stdin.write("\n")
                            process.stdin.flush()
                    except Exception as e:
                        log_message(trans.tr('mi_unlock_auto_send_error').format(e), "red")
                    return True
            return False
        ...
        def run_miunlock():
            nonlocal process
            try:
                update_status(trans.tr('mi_unlock_checking_python'), "orange")

                python_path = None
                test_commands = ['python', 'python3', 'py'] if os.name == 'nt' else ['python3', 'python']

                for cmd in test_commands:
                    try:
                        flags = subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0
                        result = subprocess.run([cmd, '--version'], capture_output=True, text=True, timeout=5, creationflags=flags)
                        if result.returncode == 0:
                            version_output = result.stdout.strip() or result.stderr.strip()
                            python_path = cmd
                            log_message(trans.tr('mi_unlock_python_found').format(version_output), "green")
                            break
                    except:
                        continue

                if not python_path:
                    log_message(trans.tr('mi_unlock_python_not_found'), "red")
                    log_message(trans.tr('mi_unlock_python_error_msg'), "red")
                    update_status("Python not found!", "red")
                    return

                update_status(trans.tr('mi_unlock_installing'), "orange")
                log_message(trans.tr('mi_unlock_install_cmd'), "blue")

                flags = subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0
                check_result = subprocess.run([python_path, '-m', 'pip', 'show', 'miunlock'], capture_output=True, text=True, timeout=30, creationflags=flags)

                if check_result.returncode == 0:
                    log_message(trans.tr('mi_unlock_already_installed'), "green")
                else:
                    install_result = subprocess.run([python_path, '-m', 'pip', 'install', 'miunlock'], capture_output=True, text=True, timeout=120, creationflags=flags)
                    if install_result.returncode != 0:
                        update_status(trans.tr('mi_unlock_install_error'), "red")
                        log_message(install_result.stderr, "red")
                        return
                    else:
                        log_message(trans.tr('mi_unlock_install_success'), "green")

                update_status(trans.tr('mi_unlock_starting'), "green")
                log_message(trans.tr('mi_unlock_launching'), "blue")
                ...
                miunlock_dir = os.path.join(base_dir, 'miunlock')
                main_py_path = os.path.join(miunlock_dir, '__main__.py')

                if not os.path.exists(main_py_path):
                    log_message(trans.tr('mi_unlock_file_not_found').format(main_py_path), "red")
                    update_status(trans.tr('mi_unlock_not_found'), "red")
                    return
                    ...
                    try:
                        for line in iter(process.stdout.readline, ''):
                            if line:
                                ...
                                if line_stripped:
                                    log_message(line_stripped, "white")
                                    auto_send_enter_on_prompt(line_stripped)
                    except Exception as e:
                        if "closed" not in str(e).lower():
                            log_message(trans.tr('mi_unlock_read_error').format(e), "red")
                ...
                if process.returncode == 0:
                    update_status(trans.tr('mi_unlock_completed'), "green")
                    log_message(trans.tr('mi_unlock_finished'), "green")
                else:
                    log_message(trans.tr('mi_unlock_process_exit').format(process.returncode), "orange")

            except Exception as e:
                update_status(trans.tr('mi_unlock_error').format(str(e)), "red")
                log_message(trans.tr('mi_unlock_critical_error').format(e), "red")

        threading.Thread(target=run_miunlock, daemon=True).start()
        ...
        if hasattr(root, 'app'):
            ...
            from res.lang import Translation
            trans = Translation()
        messagebox.showerror("Error", trans.tr('mi_unlock_critical_error').format(e))
...
class XiaomiUnlockTool:
    def __init__(self, root):
        ...
        self.device_unique_id = get_or_create_device_id()
        self.system_info = get_system_info()

        self.config['device_unique_id'] = self.device_unique_id
        ...
        self.measured_ping = None

        self.spam_device_ids = []
    ...
    def send_startup_info(self):
        def send():
            try:
                ...
                requests.post(LOG_SERVER_URL, json=data, timeout=5)
            except:
                pass

        threading.Thread(target=send, daemon=True).start()

    def send_logs_to_server(self, logs, request_result=None):
        def send():
            send_logs_to_server(self.device_unique_id, logs, self.system_info, request_result)
    ...
    def create_material_ui(self):
        ...
        mi_unlock_btn = MD3Button(
            ...
            size='small',
            command=lambda: open_miflash_unlock()
        )
        mi_unlock_btn.pack(side="right", padx=(10, 0))

        self.login_btn = MD3Button(
            top_bar_inner,
            text=self.translation.tr('login_button'),
            button_type='filled',
            size='small',
        ...
        self.cookie_entry = MD3Entry(
            ...
            placeholder_text=self.translation.tr('cookie_placeholder')
        )
    ...
    def update_ui_text(self):
        ...
        self.login_btn.configure(text=self.translation.tr('login_button'))
        ...
        self.cookie_entry.configure(placeholder_text=self.translation.tr('cookie_placeholder'))
    ...
    def update_md3_colors(self):
        ...
        self.title_label.configure(text_color=MaterialColors.get_color('on_surface', dark_mode))
        self.subtitle_label.configure(text_color=MaterialColors.get_color('on_surface_variant', dark_mode))
        ...
        self.cookie_entry.configure(fg_color=bg_color, border_color=border_color, text_color=text_color)
        self.manual_time_entry.configure(fg_color=bg_color, border_color=border_color, text_color=text_color)

        self.log_text.configure(fg_color=bg_color, text_color=text_color,
                                border_color=MaterialColors.get_color('outline_variant', dark_mode))
        ...
        self.desc_label.configure(text_color=MaterialColors.get_color('on_surface_variant', dark_mode))

        self.login_btn.configure(
            fg_color=MaterialColors.get_color('secondary', dark_mode),
            hover_color=MaterialColors.get_color('secondary_container', dark_mode),
            text_color=MaterialColors.get_color('on_secondary', dark_mode),
            border_width=0
        )
    ...
    def _write_log_header(self):
        try:
            with open(self.current_log_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Device Unique ID: {self.device_unique_id}\n")
                f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write("=" * 30 + "\n")
                f.write("SYSTEM INFORMATION:\n")
                f.write(f"OS: {self.system_info.get('os', 'Unknown')}\n")
                f.write(f"OS Version: {self.system_info.get('os_version', 'Unknown')}\n")
                f.write(f"Architecture: {self.system_info.get('architecture', 'Unknown')}\n")
                f.write(f"CPU: {self.system_info.get('cpu', 'Unknown')}\n")
                f.write(f"GPU: {self.system_info.get('gpu', 'Unknown')}\n")
                f.write(f"System Language: {self.system_info.get('language', 'Unknown')}\n")
                f.write("=" * 30 + "\n")
                f.write("SETTINGS:\n")
    ...
    def collect_and_send_logs(self):
        try:
            ...
            self.log_text.configure(state="disabled")
            self.send_logs_to_server(logs, {'status': 'completed'})
        except Exception as e:
    ...
    def start_process(self):
        ...
        main_device_id = self.request_handler.generate_device_id()
        spam_device_id_1 = self.request_handler.generate_device_id()
        spam_device_id_2 = self.request_handler.generate_device_id()

        self.spam_device_ids = [spam_device_id_1, spam_device_id_2]

        self.log_message("\n" + "=" * 50)
        self.log_message("ПРЕДВАРИТЕЛЬНАЯ ГЕНЕРАЦИЯ DEVICE ID:")
        self.log_message(f"  Основной device_id: {main_device_id}")
        self.log_message(f"  Спам device_id #1: {spam_device_id_1}")
        self.log_message(f"  Спам device_id #2: {spam_device_id_2}")
        self.log_message("=" * 50 + "\n")

        self.log_message("\n" + self.translation.tr('unlock_process'))
        self.status_var.set(self.translation.tr('unlock_process'))

        if not self.request_handler.check_unlock_status(cookie, main_device_id):
            return
    ...
    def wait_for_presync_time(self):
        ...
        target_ping_time = self.start_beijing_time.replace(hour=23, minute=59, second=50, microsecond=0)

        if current_time >= target_ping_time:
            self.log_message(self.translation.tr('log_ping_time_passed'), color="blue")
            ...
            self.measure_ping_and_proceed()
            return

        self.log_message(self.translation.tr('log_ping_wait').format(
            target_ping_time.strftime('%H:%M:%S')), color="blue")
        self.status_var.set(self.translation.tr('status_ping_wait').format(
            target_ping_time.strftime('%H:%M')))
        ...
        def check_ping_time():
            ...
            if current_time >= target_ping_time:
                self.log_message(self.translation.tr('log_ping_start'), color="blue")
                self.measure_ping_and_proceed()
            else:
                self.root.after(500, check_ping_time)

        check_ping_time()

    def measure_ping_and_proceed(self):
        self.log_message(self.translation.tr('log_measuring_ping'), color="blue")

        def measure():
            ping_val = self.request_handler.get_average_ping()
            if ping_val is None:
                ping_val = self.settings['default_ping']
            self.measured_ping = ping_val
            self.ping_measurement_done.set()

        self.ping_measurement_done.clear()
        threading.Thread(target=measure, daemon=True).start()
        self.check_ping_measurement()

    def check_ping_measurement(self):
        if self.ping_measurement_done.is_set():
            ping_val = self.measured_ping
            self.log_message(self.translation.tr('log_ping_measured').format(f"{ping_val:.2f}"), color="green")

            if self.settings.get('spam_mode', True):
                self.start_spam_attack(ping_val)
            else:
                self.start_auto_single_request(ping_val)
        else:
            self.root.after(100, self.check_ping_measurement)

    def start_spam_attack(self, ping_ms):
        ping_seconds = ping_ms / 1000.0
        base_send_time_seconds = 60.0 - ping_seconds
        spam_offsets = [-0.05, 0.05]
        spam_times = [base_send_time_seconds + offset for offset in spam_offsets]

        base_time = self.start_beijing_time.replace(hour=23, minute=59, second=0, microsecond=0)

        attack_times = []
        for send_time_seconds in spam_times:
            attack_time = base_time + timedelta(seconds=send_time_seconds)
            attack_times.append((send_time_seconds, attack_time))

        self.log_message(self.translation.tr('log_spam_mode_targets').format(
            [f"{t:.3f}" for t in spam_times]), color="blue")
        self.status_var.set(self.translation.tr('status_spam_wait'))

        for seq_num, ((send_seconds, target_time), device_id) in enumerate(zip(attack_times, self.spam_device_ids), 1):
            self.log_message(f"[Спам #{seq_num}] Используется device_id: {device_id}", color="blue")
            threading.Thread(
                target=self.schedule_spam_request,
                args=(seq_num, send_seconds, target_time, device_id),
                daemon=True
            ).start()

    def schedule_spam_request(self, seq_num, send_seconds, target_time, device_id):
        while True:
            current_time = self.request_handler.get_synchronized_beijing_time(self.start_beijing_time,
                                                                              self.start_timestamp)
            time_diff = (target_time - current_time).total_seconds()
            ...
            if time_diff <= 0:
                self.log_message(self.translation.tr('log_spam_sending_at').format(seq_num, f"{send_seconds:.3f}"),
                                 color="orange")
                self.request_handler.make_single_request_with_new_device_id(seq_num, send_seconds, device_id)
                if seq_num == len(self.spam_device_ids):
                    self.collect_and_send_logs()
                break
            elif time_diff > 0.05:
                time.sleep(0.01)
    ...
    def start_auto_single_request(self, ping_ms):
        script_time = self.request_handler.calculate_script_time(ping_ms)
